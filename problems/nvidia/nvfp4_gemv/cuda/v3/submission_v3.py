...
{
  constexpr int TMA_XACT_DATA_BYTES = BM * BK/2 + BK/2; // for a and b
  constexpr int TMA_XACT_SF_BYTES = BM * SFK + SFK; // for sfas and sfbs
  ...
  __shared__ fp8_e4m3 sfas[2][BM][SFK];
  ...
  for (int k = 0; k < K/BK; k++)
  {
    ...
    if (k + 1 < K / BK)
    {
      ...
      if (tid == 0)
      {
        ...
        if ((k + 1) % SF_K_ITER == 0)
        {
          ...
          mbarrier_arrive_expect_tx(&mbarrier[curr_idx], TMA_XACT_DATA_BYTES + TMA_XACT_SF_BYTES);
        }
        else
        {
    ...
    Reg16 sfa_reg = reinterpret_cast<Reg16*>(&sfas[((k / SF_K_ITER) % 2)][trow][tcol / 16 + (k % SF_K_ITER) * SF_PER_K_ITER])[0];
  ...
  if (tcol == 0)
  {
    __stcs(&c[0], __float2half_rn(accum));
  }
...
template<
  ...
  const int BM> // Number of elements processed along M dimension
void gemv(
  torch::Tensor a, 
  torch::Tensor b,
  torch::Tensor sfa,
  ...
  torch::Tensor c)
{
  ...
  gemv_kernel<BK, BM, WM, SFK, TK, BLOCK_SIZE><<<grid, BLOCK_SIZE>>>(
...
template void gemv<256, 16>(torch::Tensor a, torch::Tensor b, torch::Tensor sfa, torch::Tensor sfb, torch::Tensor c);
template void gemv<512, 8>(torch::Tensor a, torch::Tensor b, torch::Tensor sfa, torch::Tensor sfb, torch::Tensor c);
"""

gemv_cpp_src = """
#include <torch/extension.h>

template<const int BK, const int BM>
