  ...
  const __grid_constant__ CUtensorMap tensor_map_sfb)
{
  ...
  __shared__ uint8 as[2][BM_PER_ITER][BK/2];
  __shared__ fp8_e4m3 sfas[2][WM_ITER][BM_PER_ITER][SFK]; // need WM_ITER buffer because sfa is reused multiple times throughout K loop
  ...
  // b register is the same throughout WM_ITER
  Reg128 b_reg128; 
  Reg16 sfb_reg;
  ...
  if (tid == 0)
  {
    ...
    mbarrier_arrive_expect_tx(&mbarrier[0], TMA_XACT_DATA_BYTES + TMA_XACT_SF_BYTES);
  }
  else
  {
  ...
  {
    const int curr_col = k * BK;
    const int next_col = curr_col + BK;

    if (tid == 0 && next_col < K)
    {
      // load in next tile for b and sfb
      // cant add the col here since a and sfa still uses the same col
      ...
      if ((k + 1) % SF_K_ITER == 0)
      {
        ...
        mbarrier_expect_tx(&mbarrier[curr_idx], TMA_ALL_B_BYTES);
      }
      else
      {
        mbarrier_expect_tx(&mbarrier[curr_idx], TMA_XACT_DATA_B_BYTES);
      }
    }
    ...
    {
      const int curr_iter = k * WM_ITER + i;
      const int next_iter = curr_iter + 1;

      // load in next tile for a
      // cant load in sfa now since it is still used in this iteration
      // so load it after fma is done
      if (tid == 0 && next_iter < (K / BK) * WM_ITER)
      {        
        cp_async_bulk_tensor_2d_global_to_shared<true /*evict_first*/>(
          reinterpret_cast<uint64_t*>(&as[next_iter % 2][0][0]),
          reinterpret_cast<const uint64_t*>(&tensor_map_a),
          i == WM_ITER - 1 ? next_col / 2 : curr_col / 2,
          a_row + (i + 1) % WM_ITER * BM_PER_ITER,
          &mbarrier[next_iter % 2]);
        
        if (next_col < K && (k + 1) % SF_K_ITER == 0)
        {
          const int sf_idx = ((k + 1) / SF_K_ITER) % 2;

          cp_async_bulk_tensor_2d_global_to_shared<true /*evict_first*/>(
            reinterpret_cast<uint64_t*>(&sfas[sf_idx][i][0][0]),
            reinterpret_cast<const uint64_t*>(&tensor_map_sfa),
            next_col/16,
            a_row + i * BM_PER_ITER,
            &mbarrier[next_iter % 2]);

          mbarrier_expect_tx(&mbarrier[next_iter % 2], TMA_ALL_A_BYTES);
        }
        else
        {
          mbarrier_expect_tx(&mbarrier[next_iter % 2], TMA_XACT_DATA_A_BYTES);
        }
      }

      mbarrier_arrive(&mbarrier[curr_iter % 2]);
      parity = (curr_iter / 2) % 2;
      mbarrier_wait_parity(&mbarrier[curr_iter % 2], parity);
 
      // fma here
      Reg128 a_reg128 = reinterpret_cast<Reg128*>(&as[curr_iter % 2][trow][tcol / 2])[0];
      Reg16 sfa_reg = reinterpret_cast<Reg16*>(&sfas[((k / SF_K_ITER) % 2)][i][trow][tcol / 16 + (k % SF_K_ITER) * SF_PER_K_ITER])[0];
      
      if (i == 0)
      {
        // load into register once at the start of each WM_ITER
        b_reg128 = reinterpret_cast<Reg128*>(&bs[(k % 2)][tcol / 2])[0];
        sfb_reg = reinterpret_cast<Reg16*>(&sfbs[((k / SF_K_ITER) % 2)][tcol / 16 + (k % SF_K_ITER) * SF_PER_K_ITER])[0];
      }
